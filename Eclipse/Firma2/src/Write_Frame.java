	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.btnNewButton) {
			...
			case 0: { 
				...
				for (int i = 0; i < Datenbank_Arbeiter.worker.length; i++)
					if (Datenbank_Arbeiter.worker[i][0] == null)	{
						//SET DATA TO DATABASE
						Datenbank_Arbeiter.worker[i][0] = NameTemp;
						Datenbank_Arbeiter.worker[i][1] = LastNameTemp;
						Datenbank_Arbeiter.worker[i][2] = BirthdayTemp;
						Datenbank_Arbeiter.worker[i][3] = AdressTemp;
						Datenbank_Arbeiter.worker[i][4] = PLZTemp;
						Datenbank_Arbeiter.worker[i][5] = CityTemp;
						break;
					}	
				}
				//MESSAGE BOX
				JOptionPane.showMessageDialog(null,"Arbeiter " + NameTemp + " " + LastNameTemp + " wurde angelegt!","Neuen Arbeiter angelegt!", JOptionPane.INFORMATION_MESSAGE);
				//CLAR TEXTFIELDS
				textField.setText("");
				textField_1.setText("");
				textField_2.setText("");
			...
			case 1: { 
				//SEARCH FREE PLACE IN ARRAY
				for (int i = 0; i < Datenbank_Angestellte.oworker.length; i++)
					if (Datenbank_Angestellte.oworker[i][0] == null)	{
						//SET DATA TO DATABASE
						Datenbank_Angestellte.oworker[i][0] = NameTemp;
						Datenbank_Angestellte.oworker[i][1] = LastNameTemp;
						Datenbank_Angestellte.oworker[i][2] = BirthdayTemp;
						Datenbank_Angestellte.oworker[i][3] = AdressTemp;
						Datenbank_Angestellte.oworker[i][4] = PLZTemp;
						Datenbank_Angestellte.oworker[i][5] = CityTemp;
						break;
				}
				//MESSAGE BOX
				JOptionPane.showMessageDialog(null,"Mitarbeiter " + NameTemp + " " + LastNameTemp + " wurde angelegt!","Neuen Mitarbeiter angelegt!", JOptionPane.INFORMATION_MESSAGE);
				//CLEAR TEXTFIELDS
				textField.setText("");
				textField_1.setText("");
				textField_2.setText("");
